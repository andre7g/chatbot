...
def chat():
  ...
  print(user)
  if len(user)==0:
      user=db.insert({'user':username,'dialogo':[[]]})
      user=db.get(eid=user)
  else:
      user=user[0]
      conv=user['dialogo']
      conv.append([])
      db.update({'dialogo':conv},eids=[user.eid])
...
def receive_message(message):
    ...
    user=db.search(Usuario.user == username)
    if len(user)==0:
      user=db.insert({'usuario':username,'dialogo':[[]]})
      user=db.get(eid=user)
    else:
      user=user[0]
    ans=k.respond(message_)
    conv=user['dialogo']
    conv[-1].append({'mensaje':message_,'respuesta':ans})
    db.update({'dialogo':conv},eids=[user.eid])
...
if __name__ == '__main__':
    ...
    p.add_argument("--port",default=5000,type=int,
            ...
            help="Port url [500]")
    p.add_argument("--aiml",default="aiml/laboratorio.aiml",type=str,
            action="store", dest="aiml",
            help="AIML file with rules [aiml/laboratorio.aiml]")
